...
# 프론트엔드 업로드 경로
FRONTEND_UPLOADS_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    "..", "frontend", "public", "talismans", "uploads"
)
os.makedirs(FRONTEND_UPLOADS_DIR, exist_ok=True)

# 50 Traditional Talismans List
TALSIMAN_SEED_DATA = [
    # [금전/재물/사업/관직]
    ...
    {"category": "wish", "theme": "health", "name": "오귀퇴치부 (五鬼退治符)", "price": 15000, "desc": "주위를 맴도는 나쁜 영가(잡귀)를 쫓아내는 강력한 벽사 부적"},
...
async def generate_talisman_image(name: str) -> str:
    """Generate DALL-E 3 image and save it"""
    prompt = f"A highly detailed, authentic traditional Korean shamanic talisman (bujeok) named '{name}'. It is painted with bright red cinnabar ink on rectangular slightly yellow/golden mulberry paper (hanji). The red ink forms flowing, mystical calligraphy and talismanic symbols that look powerful and ancient. The paper is framed beautifully to look like a premium digital asset. Do not include any modern elements or texts in English. Clean white or dark premium background around the yellow paper."
    ...
    try:
        ...
        filepath = os.path.join(FRONTEND_UPLOADS_DIR, filename)
        
        img_data = requests.get(img_url).content
        with open(filepath, 'wb') as handler:
            handler.write(img_data)
            
        return f"/talismans/uploads/{filename}"
    except Exception as e:
        print(f"Failed to generate {name}: {str(e)}")
        # Fallback to existing mock image
        return "/talismans/wealth.png"

async def main():
    async with AsyncSessionLocal() as session:
        print("Starting 50 traditional talismans generation and seeding...")
        for i, data in enumerate(TALSIMAN_SEED_DATA):
            # Check if exists
            stmt = select(Product).filter(Product.name == data['name'])
            result = await session.execute(stmt)
            existing = result.scalars().first()
            
            if existing:
                print(f"[{i+1}/50] Skipped (Already Exists): {data['name']}")
                continue
            
            print(f"[{i+1}/50] Generating image for: {data['name']}...")
            image_url = await generate_talisman_image(data['name'])
            ...
            print(f"[{i+1}/50] Saved {data['name']} to database (URL: {image_url})")
            
            # Avoid rate limit
            await asyncio.sleep(2)
            
        print("All 50 talismans have been successfully generated and seeded to the DB!")
