...
def stream_multiple_tarot_readings(reading_type: str, categories: list[str]):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(10, len(categories))) as executor:
        future_to_idx = {executor.submit(process_category, i, category): i for i, category in enumerate(categories)}
        ...
        for future in concurrent.futures.as_completed(future_to_idx):
            try:
                ...
                error_result = {
                    ...
                    "card_name_kr": drawn_card["name_kr"],
                    "emoji": drawn_card["emoji"],
                    "interpretation": f"내부 오류가 발생했습니다: {str(e)}"
                }
